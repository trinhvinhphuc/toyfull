...
class Select extends Field {
	...
	public function render_control() {
		...
		<?php if ( isset( $this->args['callback'] ) && 'woodmart_get_theme_settings_html_blocks_array' === $this->args['callback'] ) : ?>
			<a href="<?php echo esc_url( admin_url( 'post.php?post=' ) ); ?>" class="xts-edit-block-link" style="display:none;" target="_blank">
		...
		<?php if ( $this->is_multiple() && ! $this->is_autocomplete() && empty( $this->args['buttons'] ) ) : ?>
			<div class="xts-select2-all-wrap">
				<a href="#" class="xts-btn xts-select2-all dashicons-insert">
					<?php esc_html_e( 'Select all', 'woodmart' ); ?>
				</a>

				<a href="#" class="xts-btn xts-unselect2-all dashicons-remove">
	...
	public function get_default_control( $options ) {
		?>
		<select class="xts-select" name="<?php echo esc_attr( $this->get_input_name() ); ?>" aria-label="<?php echo esc_attr( $this->get_input_name() ); ?>">
