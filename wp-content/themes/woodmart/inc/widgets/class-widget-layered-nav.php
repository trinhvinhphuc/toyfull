...
if ( ! class_exists( 'WOODMART_Widget_Layered_Nav' ) ) {
	class WOODMART_Widget_Layered_Nav extends WPH_Widget {
		...
		protected function layered_nav_list( $terms, $taxonomy, $query_type, $instance ) {
			...
			echo '<ul class="wd-filter-list ' . esc_attr( $class ) . '">';
			...
			foreach ( $terms as $term ) {
				...
				if ( $swatch_style || $swatch_text || $swatch_image ) {
					echo '<span style="'. $swatch_style .'" class="filter-swatch' . esc_attr( $filter_classes ) . '">';
				...
				echo '<span class="filter-name layer-term-name">' . esc_html( $term->name ) . '</span>';
		...
		protected function layered_nav_checkbox_list( $terms, $taxonomy, $query_type, $instance ) {
			...
			$classes .= ' swatches-display-' . $display;
			...
			echo '</div>';
				...
				echo '<div class="wd-pf-dropdown wd-dropdown wd-scroll">';
					echo '<ul class="wd-filter-list wd-scroll-content' . esc_attr( $classes ) . '">';
						foreach ( $terms as $term ) {
							$current_values    = isset( $_chosen_attributes[ $taxonomy ]['terms'] ) ? $_chosen_attributes[ $taxonomy ]['terms'] : array();
							$option_is_set     = in_array( $term->slug, $current_values );
							$count             = isset( $term_counts[ $term->term_id ] ) ? $term_counts[ $term->term_id ] : 0;
